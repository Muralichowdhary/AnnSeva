...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const donationsRes = await axios.get('http://localhost:3001/admin/donations', {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token")
          }
        });
        const requestsRes = await axios.get('http://localhost:3001/admin/requests', {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token")
          }
        });
  ...
  const barData = {
    ...
    datasets: [
      {
        label: 'Count',
        data: [metrics.totalDonations, metrics.totalReceivers, metrics.totalVolunteers],
        backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56'],
        hoverBackgroundColor: ['#FF6384', '#36A2EB', '#FFCE56'],
      },
    ],
